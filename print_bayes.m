...
%   distribution
    error = 1;
    precision = 10;
    datapoint_txt = append('Add data point: (', num2str(new_x, 5), ', ', num2str(new_y, 5), ')\n\n');
    
    posterior_mean_txt = append('Posterior mean:\n');
    posterior_vari_txt = append('Posterior variance:\n');
    predictive_txt = append('Predicitve distribution ~ N(', num2str(predictive_mean, 5), ', ', num2str(predictive_vari, 5), ')');
    point_end_txt = '-----------------------------------------------\n';
    
    fprintf(fileID, datapoint_txt);
    fprintf(fileID, posterior_mean_txt);
    for i=1:numel(posterior_mean)
        fprintf(fileID, append(num2str(posterior_mean(i), precision), '\n'));
    end
    fprintf(fileID, '\n');
    ...
    for i=1:I
       text = ''; 
       for j=1:J
           text = append(text, num2str(posterior_vari(i, j), precision), ', ');
